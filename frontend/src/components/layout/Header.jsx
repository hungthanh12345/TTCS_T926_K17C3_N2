...
export const Header = ({ onOpenMobileSidebar, title, subtitle }) => {
  ...
  const roles = [
    { id: 'ROLE_ADMIN', label: 'Quản trị viên (Admin)' },
    { id: 'ROLE_HR', label: 'Quản lý Nhân sự (HR)' },
    { id: 'ROLE_MENTOR', label: 'Mentor Doanh nghiệp' },
    { id: 'ROLE_STUDENT', label: 'Sinh viên Thực tập' },
  ];
  ...
  return (
    <header className="sticky top-0 z-30 h-16 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-colors">
      ...
      {/* Right: Actions, RBAC Switcher, Notifications, Avatar */}
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {/* Quick RBAC Role Testing Dropdown */}
        <div className="relative" ref={roleMenuRef}>
          <button
            onClick={() => setIsRoleMenuOpen(!isRoleMenuOpen)}
            className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-slate-100 hover:bg-slate-200/80 text-slate-700 border border-slate-200 transition-all cursor-pointer"
            title="Chuyển đổi vai trò trải nghiệm RBAC"
          >
            <Shield className="w-3.5 h-3.5 text-indigo-600" />
            <span className="truncate max-w-[120px]">{user?.role?.replace('ROLE_', '')}</span>
            <ChevronDown className="w-3 h-3 text-slate-400" />
          </button>

          {isRoleMenuOpen && (
            <div className="absolute right-0 mt-2 w-60 rounded-xl bg-white shadow-xl border border-slate-200 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="px-3 py-1.5 border-b border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  Chuyển Vai Trò (RBAC Demo)
                </p>
              </div>
              {roles.map((r) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer ${
                    user?.role === r.id ? 'font-bold text-indigo-600 bg-indigo-50/50' : 'text-slate-700'
                  }`}
                >
                  <span>{r.label}</span>
                  {user?.role === r.id && <CheckCircle2 className="w-3.5 h-3.5 text-indigo-600" />}
                </button>
              ))}
            </div>
          )}
